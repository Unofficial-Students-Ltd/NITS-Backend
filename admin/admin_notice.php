...
if ($_POST)
{
	...
	if (isset($_POST['submitnewnewsupdates_btn'])) {
		...
		$file_store = "../uploads/noticeboard/".$title.".".$extn;
		// echo "<script>alert('Update Picture');</script>";
		move_uploaded_file($file_tem_loc, $file_store);


		$query="INSERT INTO noticeboard (visible,title ,ext,bold,status) VALUES('$postvisible', '$title','$extn','$bolded','$newstatus')";
		$result= mysqli_query($connection,$query) or die(mysqli_error($connection));
		if(!$result) {echo "Unable to submit new ";}
		else echo "Submitted";

        echo "<script>window.location.href = 'http://localhost/nit2020/admin/noticeboard.php';</script>";
	...
	<body>
			...
			<form action="noticeboard.php" method="post" name="myform" >
	...
	if(isset($_POST['visible']))
		...
		if (isset($_POST['submiteditnewsupdates_btn'])) {
			...
			$file_store = "../uploads/noticeboard/".$titl.".".$extn;
			// echo "<script>alert('Update Picture');</script>";
			move_uploaded_file($file_tem_loc, $file_store);
